...
import { box, merge } from '../../maps/northreach/assets/mdl-lib.mjs';
const require = createRequire(import.meta.url);
import {encodeBLP,writeFlatTexture,writeModel,custodianParts} from './assets.mjs';
...
function unit(name,model,extra=[]){return [field('unam',name),field('umdl',model),field('uico','ReplaceableTextures\\CommandButtons\\BTNAHportrait.blp'),field('utip',name),field('utub',name),field('uaen',0),field('ufoo',0),field('ubui',''),...extra];}
...
export async function generate(out){
  ...
  info.loadingScreen={background:-1,path:'',title:'AFTER HOURS',subtitle:'Emergency Lighting — asset repair 2',text:'Right-click to move. E: interact. Q: quiet walk. R: sprint. Find 3 fuses, use the generator, then reach EXIT. Gold = team fuses; lumber = stamina. Three captures exhaust a worker. -help repeats controls.'};
  ...
  writeFlatTexture(assets);
  const make=(name,geosets,min,max)=>writeModel(assets,name,geosets,min,max);
  ...
  make('Custodian',custodianParts(),[-30,-30,0],[30,30,178]);
  ...
  for(const [name,colour] of [['portrait',[150,164,128]],['interact',[222,184,97]],['sprint',[232,213,117]],['quiet',[144,184,166]]]){
    for(const disabled of [false,true]){const pix=Buffer.alloc(64*64*3);for(let y=0;y<64;y++)for(let x=0;x<64;x++){const border=x<3||y<3||x>60||y>60;let on=false;if(name==='sprint')on=(y>9&&y<53&&Math.abs(x-(40-y*.3))<6);else if(name==='quiet')on=Math.abs(Math.hypot(x-33,(y-30)*.9)-15)<4||(x>29&&x<36&&y>32&&y<53);else on=(x>20&&x<44&&y>16&&y<49)||(x>15&&x<49&&y>26&&y<37);const rgb=border?[89,83,57]:on?colour:[28,30,23];for(let c=0;c<3;c++)pix[(y*64+x)*3+c]=Math.round(rgb[c]*(disabled?.4:1));}const iconDir=path.join(out,'imports','ReplaceableTextures',disabled?'CommandButtonsDisabled':'CommandButtons');fs.mkdirSync(iconDir,{recursive:true});fs.writeFileSync(path.join(iconDir,(disabled?'DISBTN':'BTN')+'AH'+name+'.blp'),encodeBLP(64,64,pix));}
  }
  ...
  objects.custom['uM00:hfoo']=unit('Custodian','war3mapImported\\Custodian.mdl',[field('uabi','Avul'),field('umvs',215),field('usca',1),field('ucol',16),field('uhpm',9999),field('usin',100),field('usid',100)]);
  for(const [id,name] of [['uA00','Wall'],['uB00','Carpet'],['uC00','Fixture'],['uD00','Fuse'],['uE00','Generator'],['uF00','Exit']])objects.custom[id+':hfoo']=unit(name,'war3mapImported\\'+name+'.mdl',[field('uabi','Aloc,Avul'),field('umvs',0),field('umvt','none'),field('usca',1),field('ushd',''),field('ucol',0),field('usin',0),field('usid',0)]);
  writeJson(path.join(out,'objects-units.json'),objects);
  writeJson(path.join(out,'objects-abilities.json'),{original:{},custom:{
    'aI00:ANcl':channel('Interact','E','channel','ReplaceableTextures\\CommandButtons\\BTNAHinteract.blp','Pick up a nearby fuse, restore the generator with all three fuses, or use the exit.',0.2),
    'aS00:ANcl':channel('Sprint','R','berserk','ReplaceableTextures\\CommandButtons\\BTNAHsprint.blp','Run for 4 seconds. Costs 50 stamina; 10-second cooldown. Loud enough to attract the Custodian.',10),
    'aQ00:ANcl':channel('Quiet Walk','Q','roar','ReplaceableTextures\\CommandButtons\\BTNAHquiet.blp','Toggle slower, quieter movement. The Custodian hears you from a much shorter distance.',0.2)}});
  ...
  writeJson(path.join(out,'imports-credits.json'),{entries:{'war3mapImported/':{author:'Serendipity / wc3-map-toolkit',resource:'Original generated geometry, UI graphics and electrical hum',source:'examples/after-hours/generate.mjs',license:'Original generated assets; MIT',fetched:'2026-09-15'},'ReplaceableTextures/':{author:'Serendipity / wc3-map-toolkit',resource:'Original generated BLP1 button icons',source:'examples/after-hours/generate.mjs',license:'Original generated assets; MIT',fetched:'2026-09-15'}}});
  const importRoot=path.join(out,'imports');const allImports=require('../../lib/source').walk(importRoot);
  writeJson(path.join(out,'asset-permissions.json'),{entries:allImports.sort().map(name=>({path:name,sha256:crypto.createHash('sha256').update(fs.readFileSync(path.join(importRoot,name))).digest('hex'),redistributionPermitted:true,authority:'Original procedural art and composed audio generated by examples/after-hours/generate.mjs and assets.mjs; Serendipity; MIT distribution.'}))});
