...
const os = require('os');
const policy = require('./extraction-policy');
...
function safeMemberRel(name) { return policy.memberRel(name); }
...
function slListNames(SL, hMpq) {
  ...
  try {
    while (cur) {
      if (names.length >= policy.DEFAULT_LIMITS.maxEntries) throw policy.refusal('archive enumeration entry budget exceeded');
      names.push(cur.cFileName);
      cur = SL.SFileFindNextFile(first.hFind);
    }
  } finally { SL.SFileFindClose(first.hFind); }
  return names;
}
...
function slReadMember(SL, E, hMpq, name, budget) {
  ...
  try {
    ...
    (budget ? budget.consume : (n => { if (!Number.isSafeInteger(n) || n < 0 || n > policy.DEFAULT_LIMITS.maxFileBytes) throw policy.refusal('member byte budget exceeded'); }))(size);
...
function smpqListRaw(abs) {
  ...
  let out;
  try {
    out = execFileSync('smpq', ['-l', abs], { encoding: 'utf8', timeout: 60000, maxBuffer: 16 * 1024 ** 2 });
  } catch (e) {
    throw new Error(`smpq -l failed on ${abs}: ${e.message}`);
  }
  // Output lines: "<size> <date> <time> <name>"
  const lines = out.split(/\r?\n/).filter(l => l.trim());
  if (lines.length > policy.DEFAULT_LIMITS.maxEntries) throw policy.refusal('archive enumeration entry budget exceeded');
  return lines.map(l => { const m = /^\s*\d+\s+\S+\s+\S+\s(.*)$/.exec(l); if (!m) throw policy.refusal('unrecognised smpq listing'); return m[1]; });
}
...
function extractAll(archivePath, outDir, opts) {
  ...
  const abs = path.resolve(archivePath);
  const budget = policy.context(outDir, opts.limits);
  policy.checkedArchive(abs, budget.limits);
  ...
  if (sl) {
    ...
    try {
      ...
      for (const name of names) {
        if (!SL.SFileHasFile(hMpq, name)) continue;
        const rel = budget.reserve(name);
        const buf = slReadMember(SL, E, hMpq, name, budget);
        if (buf === null) continue;
        budget.write(rel, buf);
        extracted.push(rel);
      }
      if (dumpUnknown) {
        ...
        for (const name of all.filter((n) => PSEUDO_NAME.test(n))) {
          ...
          } catch (e) { if (e.code === 'WC3_EXTRACT_SAFETY') throw e; continue; }
          if (dedup.isDuplicate(buf)) continue; // same content probed by name
          const rel = budget.reserve(`_unknown/${unknownDumpName(name, buf)}`, true);
          budget.write(rel, buf);
          unknown.push(rel);
        }
      }
    ...
    return { ...extractResult(extracted, all.length, unknown), enumeration: enumerationEvidence(all, extracted), limits: budget.limits };
  }

  assertSmpq();
  let all = [];
  try { all = smpqListRaw(abs); } catch (e) { if (e.code === 'WC3_EXTRACT_SAFETY') throw e; /* conservative unknown counts */ }
  ...
  const names = unionNames(all.filter((n) => !PSEUDO_NAME.test(n)), KNOWN_FILES);
  smpqExtractBatch(abs, names, outDir, extracted, budget);
  ...
  if (dumpUnknown) {
    ...
    if (pseudo.length > 0) {
      ...
      try {
        const dedup = extractedContentIndex(outDir, extracted);
        const got = [];
        smpqExtractBatch(abs, pseudo, staging, got, policy.context(staging, budget.limits));
        for (const rel0 of got) {
          const buf = fs.readFileSync(path.join(staging, rel0));
          if (dedup.isDuplicate(buf)) continue;
          budget.consume(buf.length);
          const rel = budget.reserve(`_unknown/${unknownDumpName(rel0, buf)}`, true);
          budget.write(rel, buf);
          unknown.push(rel);
        }
      } finally {
...
// order.
// Exact-name extraction into a private, fresh workspace. One member per
// bounded subprocess permits enforcing the aggregate budget before publication.
// smpq wildcard syntax and option-looking names are rejected by memberRel.
function smpqExtractBatch(abs, names, dir, sink, budget) {
  budget ||= policy.context(dir);
  if (process.platform === 'win32') throw policy.refusal('bounded smpq extraction requires POSIX resource limits; use native StormLib on Windows');
  const stage = fs.mkdtempSync(path.join(os.tmpdir(), 'wc3-smpq-'));
  try {
    // External names teach StormLib's list operation which hashes to resolve;
    // enumerate once instead of spawning a process for each absent probe.
    const external = path.join(stage, 'names.txt');
    if (names.length > budget.limits.maxEntries) throw policy.refusal('candidate entry budget exceeded');
    for (const name of names) if (!policy.memberRel(name)) throw policy.refusal(`unsafe candidate: ${name}`);
    fs.writeFileSync(external, names.join('\n') + '\n');
    const listed = execFileSync('smpq', ['-l', '-L', external, abs], {
      encoding: 'utf8', timeout: budget.limits.timeoutMs, maxBuffer: 16 * 1024 ** 2 });
    const resolved = new Map();
    for (const line of listed.split(/\r?\n/).filter(l => l.trim())) {
      const match = /^\s*(\d+)\s+\S+\s+\S+\s(.*)$/.exec(line);
      if (!match || !policy.memberRel(match[2])) throw policy.refusal('unsafe or unrecognised external smpq listing');
      if (resolved.size >= budget.limits.maxEntries) throw policy.refusal('external listing entry budget exceeded');
      const key = policy.identity(match[2]);
      if (resolved.has(key)) throw policy.refusal(`ambiguous smpq entry: ${match[2]}`);
      resolved.set(key, Number(match[1]));
    }
    for (const name of names) {
      const safe = policy.memberRel(name);
      if (!resolved.has(policy.identity(safe))) continue;
      if (!safe) throw policy.refusal(`unsafe smpq member: ${name}`);
      const work = fs.mkdtempSync(path.join(stage, 'member-'));
      const remaining = Math.min(budget.limits.maxFileBytes, budget.limits.maxTotalBytes - budget.bytes);
      if (remaining < 1) throw policy.refusal('extraction total byte budget exhausted');
      if (resolved.get(policy.identity(safe)) > remaining) throw policy.refusal('declared member exceeds extraction byte budget');
      try {
        execFileSync('python3', [path.join(__dirname, '../scripts/bounded-smpq.py'), String(remaining),
          String(Math.ceil(budget.limits.timeoutMs / 1000)), '-x', abs, name],
          { cwd: work, timeout: budget.limits.timeoutMs, maxBuffer: 1024 ** 2 });
      } catch (e) {
        // A missing exact name is a normal probe miss; resource/process failures
        // are not. smpq ordinarily returns success even for absent names.
        throw policy.refusal(`bounded smpq process failed for ${name}: ${e.message}`);
      }
      const candidate = path.join(work, ...safe.split('/'));
      if (fs.existsSync(candidate)) {
        policy.checkAncestors(path.dirname(candidate));
        const info = policy.hashFile(candidate, remaining);
        if (info.size !== resolved.get(policy.identity(safe))) throw policy.refusal('smpq output size disagrees with listing');
        const rel = budget.reserve(safe); budget.consume(info.size);
        budget.write(rel, fs.readFileSync(candidate)); sink.push(rel);
      }
      fs.rmSync(work, { recursive: true, force: true });
    }
  } finally { fs.rmSync(stage, { recursive: true, force: true }); }
}
function screenNames(names, budget) {
  if (names.length > budget.limits.maxEntries) throw policy.refusal('archive entry budget exceeded');
  const seen = new Map();
  for (const name of names) {
    const rel = policy.memberRel(name);
    if (!rel) throw policy.refusal(`unsafe archive member: ${JSON.stringify(name)}`);
    const key = policy.identity(rel);
    if (seen.has(key)) throw policy.refusal(`ambiguous archive member (duplicate or locale): ${name}`);
    seen.set(key, name);
  }
}
function enumerationEvidence(all, extracted) {
  // Do not let name probing or equal payload hashes prove anonymous-entry
  // identity. Protected archives remain incomplete until independently mapped.
  const named = all.filter(n => !PSEUDO_NAME.test(n));
  const pseudo = all.filter(n => PSEUDO_NAME.test(n));
  const got = new Set(extracted.map(policy.identity));
  const missing = named.filter(n => !got.has(policy.identity(n)));
  return { observed: all.length > 0, named: named.length, anonymous: pseudo.length,
    missing, complete: all.length > 0 && pseudo.length === 0 && missing.length === 0 && named.length === got.size,
    scope: 'StormLib-enumerable default-locale entries; not a proof against library/parser bugs' };
}
...
function probeExtract(archivePath, outDir, names, opts = {}) {
  const abs = path.resolve(archivePath);
  const budget = policy.context(outDir, opts.limits);
  policy.checkedArchive(abs, budget.limits);
  ...
  if (sl) {
    ...
    try {
      for (const { name, rel } of todo.values()) {
        let buf = null;
        try { buf = slReadMember(SL, E, hMpq, name, budget); } catch (e) { if (e.code === 'WC3_EXTRACT_SAFETY') throw e; continue; }
        if (buf === null) continue;
        budget.reserve(rel); budget.write(rel, buf); recovered.push(rel);
      }
    } finally {
      SL.SFileCloseArchive(hMpq);
    }
  ...
  assertSmpq();
  smpqExtractBatch(abs, [...todo.values()].map((t) => t.name), outDir, recovered, budget);
  return { probed: todo.size, recovered };
}
