...
//     write). flags = named-bits | flagsUnknown. When the key is absent
//     (hand-written JSON), the writer mirrors upstream and force-sets
//     0x400|0x4000|0x8000.
...
function warToJson(buffer) {
  ...
  if (flagsUnknown !== 0) map.flagsUnknown = flagsUnknown;
  ...
  result.map = {
    ...
    ...(map.flagsUnknown !== undefined ? { flagsUnknown: map.flagsUnknown } : {}),
  };
  ...
  const availablePlayerNums = result.players.map((p) => p.playerNum);
