...
//
// TRAILING-ZERO-DWORD DIALECT (`"_trailer": "00000000"` sidecar): a large
// slice of real maps ships ONE extra all-zero u32 after the custom table.
// Measured across nine unrelated maps by six different Korean authors, six
// were affected (23 of their 43 object files: meandyou 6/6, buildblock 5/5,
// yutnori 4/4, bbulle 3/3, turncard 3/3, sugeon 2/3) — and those files
// correlate perfectly
// with a distinct writer lineage that also zeroes EVERY modification's
// trailing sanity-check dword (100% of original-table modifications in
// every affected file, versus the conventional base-id trailer in the
// unaffected ones). It carries no data: mdx-m3-viewer-th and the game both
// stop reading after the custom table and ignore whatever follows. Refusing
// it made map-to-json fall back to a raw copy, which silently reported zero
// object-data text for maps holding hundreds of thousands of characters.
// So: READ tolerates it and records it as a `_trailer` sidecar on the
// emitted JSON (same contract as lib/wts.js's `_dialect`, gotcha 16 — it is
// not an object entry), and WRITE consumes the sidecar and re-emits the
// bytes, keeping the round-trip byte-exact. Deleting the sidecar is safe:
// the write path then emits nothing extra.
// BOUNDARY (deliberate, do not widen): ONLY a trailing dword of exactly
// four 0x00 bytes is tolerated. Any other trailing bytes — a different
// length, or four non-zero bytes — still throw. That error is a real
// corruption / mis-parse signal (a drifted cursor, a truncated record, a
// protector stub) and weakening it would hide genuine damage.
...
// The only tolerated post-custom-table remainder, and the sidecar value that
// records it. Exactly four 0x00 bytes — see TRAILING-ZERO-DWORD DIALECT above.
const ZERO_TRAILER = Buffer.alloc(4);
const TRAILER_MARK = '00000000';
...
// Returns { version, trailer } — trailer true when the buffer carried the
// all-zero trailing dword.
function readBuffer(type, buffer, result) {
  ...
  readTable('original');
  readTable('custom');
  const rest = buffer.length - c.off;
  let trailer = false;
  if (rest === 4 && buffer.subarray(c.off).equals(ZERO_TRAILER)) {
    trailer = true; // known dialect, recorded as the `_trailer` sidecar
    c.off += 4;
  } else if (rest !== 0) {
    // deliberately NOT widened: anything but a single all-zero dword is a
    // genuine corruption / mis-parse signal
    throw new Error(`objects2: ${rest} unexpected trailing byte(s) after the custom table`);
  }
...
function warToJson(type, buffer, skinBuffer) {
  ...
  const { version, trailer } = readBuffer(type, buffer, result);
  if (version === 3) {
    throw new Error('objects2: main file is v3 — it belongs to the upstream translator');
  }
  // NB: v1/v2-era maps have no war3mapSkin.* twins (the split is a 1.32+
  // container convention), so only the MAIN buffer's trailer is recorded;
  // a skin buffer's own trailer is tolerated on read and not round-tripped.
  if (skinBuffer) readBuffer(type, skinBuffer, result); // merge, like upstream
  const json = { version };
  if (trailer) json._trailer = TRAILER_MARK;
  json.original = result.original;
  json.custom = result.custom;
  return { json };
}
...
function jsonToWar(type, json) {
  ...
  // `_trailer` sidecar (see TRAILING-ZERO-DWORD DIALECT): re-emit the source
  // file's all-zero trailing dword so its round-trip stays byte-exact. Absent
  // key (incl. a user who deleted it) = emit nothing, the WE-default layout.
  if (json._trailer !== undefined && json._trailer !== null) {
    if (json._trailer !== TRAILER_MARK) {
      throw new Error(`objects2: "_trailer" must be "${TRAILER_MARK}" (the only tolerated trailing dword) or absent, got ${JSON.stringify(json._trailer)}`);
    }
    w.raw(ZERO_TRAILER);
  }
