...
function sourceToExtracted(sourceDir, outDir) {
  ...
  // 2. Copy scripts verbatim — except war3map.lua, which gets the generated
  //    CreateAllUnits() block appended from units.json (war3mapUnits.doo is
  //    editor-only data; the game only creates script-created units).
  //    The block is marker-delimited and stripped before regeneration, so
  //    repacking an extracted source is stable. JASS (war3map.j) sources are
  //    copied untouched — write CreateAllUnits yourself there.
  const unitsJson = readSourceJson('units.json');
  for (const s of SCRIPT_FILES) {
    const p = path.join(sourceDir, s);
    if (!fs.existsSync(p)) continue;
    if (s === 'war3map.lua' && unitsJson) {
      const lua = injectUnitsIntoLua(fs.readFileSync(p, 'utf8'), unitsJson);
