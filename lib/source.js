...
const { isClassicDooOverread, explain: explainClassicDoo } = require('./classicdoo');
const { generateMinimapTGA, generateMmp } = require('./minimap');
const { generateWpm, generateShd, checkPathingFile } = require('./pathing');
...
function extractedToSource(extractedDir, sourceDir) {
  ...
  for (const rel of all) {
    ...
    if (base && CONSUMED_AS_SKIN.has(base)) continue; // defer until parent conversion outcome is known
    ...
    if (entry) {
      ...
      if (trap) {
        ...
        try {
          ...
          if (skinBuf) consumedSkins.add(entry.skinWar);
          continue;
        } catch (e) {
          // classic-format or corrupt file: fall through to opaque copy.
          // The pre-1.32 .doo layout is an IDENTIFIED, benign case (upstream
          // reads a 1.32+-only skinId per entry and overreads — lib/
          // classicdoo.js proves the classic layout consumes the file
          // exactly), so it is named in the manifest instead of reading as
          // an unexplained corruption. Same shape as the trap-stub note.
          const classicDoo = isClassicDooOverread(base, raw, e);
          manifest.errors.push(classicDoo
            ? { file: rel, error: String(e.message || e), note: explainClassicDoo(classicDoo) }
            : { file: rel, error: String(e.message || e) });
  ...
  // A companion is consumed only when its parent's conversion completed.
  // Orphan skins and skins belonging to rejected/trap parents otherwise
  // vanished silently. Defer this pass so directory enumeration order cannot
  // change whether raw bytes survive. Never parse an unconsumed companion.
  for (const rel of all) {
    if (!CONSUMED_AS_SKIN.has(rel) || consumedSkins.has(rel)) continue;
    writeFileP(path.join(sourceDir, 'files', rel), fs.readFileSync(path.join(extractedDir, rel)));
    manifest.copied.push(rel);
    manifest.errors.push({
      file: rel,
      error: 'skin companion not merged: parent absent or conversion did not complete',
      note: 'copied through raw; original companion bytes preserved',
    });
  }
...
// Returns { written: [relpaths], header: {..}|null, warnings: [strings] }.
// opts.variantName overlays the internal map name (w3i map.name, resolving
// any TRIGSTR indirection to the plain string) at TRANSLATION time — the
// source directory is never modified (gotcha 17 / A-B variants; build-map
// applies the same name to the HM3W header fields).
function sourceToExtracted(sourceDir, outDir, opts) {
  opts = opts || {};
  ...
  for (const ent of rootEntries) {
    ...
    if (!entry) continue;
    let json = readJson(path.join(sourceDir, ent.name));
    if (opts.variantName && entry.json === 'info.json' && json && json.map) {
      // Variant overlay: a plain string here overrides (and resolves) any
      // TRIGSTR_n indirection — the game shows the w3i name as stored.
      json = { ...json, map: { ...json.map, name: opts.variantName } };
    }
    json = normalizeForWar(entry, json);
  ...
  // 6. Path map + shadow map: sizes derive from terrain dimensions
  //    (gotcha 8). Generate all-passable / no-shadow defaults when the
  //    source ships neither under files/; NEVER clobber provided pathing
  //    data — but warn when a provided copy doesn't match terrain.json
  //    (the classic resize foot-gun; lib/pathing.js).
  if (terrainJson) {
    for (const [name, generate] of [['war3map.wpm', generateWpm], ['war3map.shd', generateShd]]) {
      if (written.includes(name)) {
        const provided = path.join(sourceDir, 'files', name);
        if (fs.existsSync(provided)) {
          const w = checkPathingFile(name, fs.readFileSync(provided), terrainJson);
          if (w) warnings.push(w);
        }
      } else {
        writeFileP(path.join(outDir, name), generate(terrainJson));
        written.push(name);
      }
    }
  }

  // 7. Header fields pass through for the packer.
  let header = null;
  const hdrPath = path.join(sourceDir, '_header.json');
  if (fs.existsSync(hdrPath)) header = readJson(hdrPath);

  return { written, header, warnings };
}
