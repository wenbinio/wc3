...
function createVM() {
  ...
  const vm = {
    ...
    // Opt-in line-event hook for script coverage (lib/sim/coverage.js).
    // Fires onLine(line) for every VM line event whose chunk source equals
    // `chunkname` (the packed map script — harness/test chunks are filtered
    // out). Pure observation: the callback sees only a line number and the
    // hook touches no Lua-visible state. Never installed unless coverage is
    // requested, so normal runs (and golden runs) carry no hook at all.
    installLineHook(chunkname, onLine) {
      const want = to_luastring(chunkname);
      lua.lua_sethook(L, function (Ls, ar) {
        if (ar.event !== lua.LUA_HOOKLINE) return;
        lua.lua_getinfo(Ls, to_luastring('S'), ar);
        const src = ar.source;
        if (!src || src.length !== want.length) return;
        for (let i = 0; i < want.length; i++) if (src[i] !== want[i]) return;
        onLine(ar.currentline);
      }, lua.LUA_MASKLINE, 0);
    },
