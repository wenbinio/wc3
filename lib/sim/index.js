...
const { lineReport, registerRunDump, ENV_VAR: LINECOV_ENV } = require('./coverage');
...
//   lineCoverage true to collect script LINE coverage (lib/sim/coverage.js);
//                default OFF — no debug hook is installed on a normal run.
//                Also auto-enabled (with an exit-time dump for aggregation)
//                when the WC3_SIM_LINECOV_DIR env var names a directory —
//                that is how tools/test-map-logic.js --coverage observes
//                the map's own test suite.
function loadMap(target, opts) {
  ...
  // ---- opt-in line coverage (observes only; no hook on a normal run) ------
  const linecovDir = process.env[LINECOV_ENV];
  let lineHits = null;
  if (opts.lineCoverage === true || (linecovDir && mapDir)) {
    lineHits = new Set();
    vm.installLineHook(chunkname, (line) => lineHits.add(line));
    if (linecovDir && mapDir) registerRunDump(path.resolve(mapDir), lineHits, linecovDir);
  }
  ...
  // plus (when opts.lineCoverage / WC3_SIM_LINECOV_DIR enabled it) script
  // LINE coverage of the packed chunk mapped back to SOURCE war3map.lua
  // lines: { source: {executable, hit, pct, missedRanges}, generated:
  // {executable, hit} } — null when line coverage was not requested.
  sim.coverage = function coverage() {
    ...
    return {
      ...
      lines: lineHits ? lineReport(script, lineHits) : null,
    };
  };
...
module.exports = { loadMap, LuaError, fourCC, fourCCToStr, assemblePackedLua };
