...
// Some protectors overwrite MPQ v1 header fields the game itself never
// checks — observed in the wild: headerSize 0x200102 and headerSize 'VxOP'
// (0x504F7856) where a v1 header is always 0x20. StormLib ignores the field
// for v1 archives; normalize it on the COPY handed to the viewer so no code
// path trusting headerSize can misbehave. The sector-size exponent is left
// alone deliberately: the viewer honors it for sector math (512 << exp) and
// enumerates such archives fine (verified on a real exp=15 map) — "fixing"
// it would corrupt reads of any archive genuinely using bigger sectors.
function normalizeMpqHeaderCopy(u8) {
  for (let o = 0; o + 32 <= u8.length; o += 512) {
    if (u8[o] === 0x4D && u8[o + 1] === 0x50 && u8[o + 2] === 0x51 && u8[o + 3] === 0x1A) { // 'MPQ\x1a'
      const dv = new DataView(u8.buffer, u8.byteOffset + o, 32);
      const formatVersion = dv.getUint16(12, true);
      if (formatVersion === 0 && dv.getUint32(4, true) !== 0x20) dv.setUint32(4, 0x20, true);
      return u8;
    }
  }
  return u8; // no MPQ magic: let the viewer produce its own error
}

// Open a whole .w3x/.mpq with the viewer's MPQ parser, readonly.
// `buffer` is a Node Buffer; a fresh Uint8Array copy is made here (and
// nonstandard header fields are normalized on that copy — never the input).
function openMapReadonly(buffer) {
  const map = new War3Map();
  map.load(normalizeMpqHeaderCopy(new Uint8Array(buffer)), true); // readonly: never use its save path
  return map;
}
