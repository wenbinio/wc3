...
- **game-ai-principles.md** (2026-09-05) — what does the established game-AI discipline say an engineer should know BEFORE designing an RTS-map AI, and where did the Rome programme diverge from it? Web-researched from primary sources (Game AI Pro 1–3 + Online 2021, Orkin/Isla/Pottinger/Emerson GDC papers, Mark & Dill, Lewis, Wetzel & Anderson, Ocio, Skarupke, Johnson, Total War and Steamhammer postmortems), every claim labelled DOC / PRAC / RES / PROG / SYN. Nine sections: architectures compared with utility's pathologies sourced; selection vs execution; hysteresis and the feedback-loop trap (the programme's three instances); world model and scale; movement and space; pacing and legibility with the numbers (0.2/0.4 s, the perception correlations); testing the closed loop; an 18-entry ranked foible catalogue; a 15-question start-here checklist. Sits ABOVE `wc3-ai-lessons.md` and cites it rather than restating it; says plainly where the literature contradicts the programme's record. **Staleness**: technique sources age slowly; Total War / Steamhammer numbers are patch values; programme citations are pinned to DESIGN.md §1–§32 and build `65a3ce2`. The Ontañón survey and Dill's design-patterns paper could not be fetched and are cited by reference only.
